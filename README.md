...
└── src/
    ...
    ├── api/
    │   ├── client.ts        Shared Axios client + request/response interceptors
    │   ├── authApi.ts       Real JWT login against DummyJSON
    │   ├── tokenStore.ts    SecureStore wrapper for the JWT (web fallback)
    │   ├── cloudApi.ts      Real GET + write against JSONPlaceholder
    │   └── statsApi.ts      Real GET, then local analytics aggregation
    ├── hooks/
    │   ├── useAuth.ts
    │   └── useTheme.ts
...
- **Networking (real HTTP).** `api/cloudApi` does a GET (`/posts`, used by the
  ...
  `api/statsApi` performs the real GET and then aggregates the local session
  data into the analytics summary.
...
- The Stats screen uses TanStack Query; its loading / error+retry / success
  branches reflect the real network call. Auto-retry is disabled so the error
